...
public class TimeBasedOneTimePassword {
    ...
    private static final String SHARED_SECRET = "JBSWY3DPEHPK3PXP";
    private static final int VARIANCE = 5;

    public static boolean isVerificationCodeValid(int code) throws InvalidCodeException {
        boolean result = false;
        try {
            result = verifyCode(SHARED_SECRET, code, getTimeIndex(), VARIANCE);
        } catch (NoSuchAlgorithmException e) {
            throw new InvalidCodeException("invalid algorithm");
        } catch (InvalidKeyException e) {
            throw new InvalidCodeException("invalid key");
        }
